...
window.onload = function() {
    ...
    // Uygulama açılışında verileri ÇEKMİYORUZ. Giriş yapıldığında çekilecek.
    ...
    db.ref(DB_REF).once('value').then((snapshot) => {
        ...
        if(isFirstLoad) {
            ...
            if(loginMode === 'teacher') {
                ...
                let headerCode = document.getElementById('headerClassCode');
                if(headerCode && currentClassCode) {
                    headerCode.innerText = `Sınıf Kodu: ${currentClassCode}`;
                    headerCode.style.display = 'inline-block';
                }
            } else {
...
function updateUI() { 
    try {
        ...
        // Update stats explicitly here so they always display regardless of tab rendering
...
function renderBookManager() {
    ...
    filtered.forEach((b, index) => {
        ...
        if(isEditMode) {
            let encodedB = b.replace(/'/g, "\\'").replace(/"/g, '&quot;');
            contentHtml = `<div style="display:flex; flex-direction:column; gap:10px;">
                <div style="display:flex; align-items:center; gap:10px;">
                    <input type="checkbox" class="book-delete-checkbox" value="${encodedB}" style="transform: scale(1.5);">
                ...
                <div style="display:flex; gap:10px;">
                    ...
                    <button class="btn btn-danger" style="margin:0; width:auto;" onclick="event.stopPropagation(); delSingleBook('${encodedB}')"><i class="fas fa-trash"></i></button>
                </div>
            </div>`;
        } else {
...
function login() {
    if(loginMode === 'teacher') {
        ...
        document.getElementById('loader').style.display = 'block';
        document.getElementById('loader').innerText = "Giriş Yapılıyor...";
        
        auth.signInWithEmailAndPassword(email, pass).then((userCredential) => {
            let uid = userCredential.user.uid;
            
            // Check for class code or generate one
            db.ref('/teacherCodes/' + uid).once('value').then((snapshot) => {
                if (snapshot.exists()) {
                    currentClassCode = snapshot.val();
                    loadClassData(uid);
                } else {
                    currentClassCode = generateClassCode();
                    db.ref('/teacherCodes/' + uid).set(currentClassCode);
                    db.ref('/classCodes/' + currentClassCode).set(uid).then(() => {
                        loadClassData(uid);
                    });
                }
            });
        ...
        let code = document.getElementById('classCodeInput').value.trim();
        let sPass = document.getElementById('studentLoginPass').value.trim();
        
        if(!code || !sPass) return alert("Sınıf kodu ve şifre zorunludur.");
        
        document.getElementById('loader').style.display = 'block';
        document.getElementById('loader').innerText = "Sınıf Aranıyor...";
        
        db.ref('/classCodes/' + code).once('value').then((snapshot) => {
            if (snapshot.exists()) {
                let uid = snapshot.val();
                DB_REF = '/classes/' + uid;
                
                // Fetch only studentPass list first to verify
                db.ref(DB_REF + '/studentPass').once('value').then((passSnapshot) => {
                    let passes = passSnapshot.val() || {};
                    let foundStudent = Object.keys(passes).find(key => String(passes[key]).trim() === String(sPass));
...
function getTargetStudents(sVal) {
    let cb = document.getElementById('allClassCheck');
    if (cb && cb.checked) {
...
function processBulkReturns() {
    ...
    // First background return them all so they are properly marked in case of skip
    let timeStr = getLocalTime();
    selectedIds.forEach(id => {
        let rec = records.find(r => r.id === String(id));
        if (rec) {
            rec.status = "İade Etti";
            rec.returnDate = timeStr;
        }
    });
    updateUI();
    syncData();

    // Set global queue for ratings
...
function toggleEditMode() { 
    ...
    document.getElementById('editToggleBtn').classList.toggle('active'); 
    document.getElementById('editToggleBtn').innerText = isEditMode ? '✅ Bitir' : '✏️ Düzenle'; 
    document.getElementById('bulkDeleteBooksBtn').style.display = isEditMode ? 'block' : 'none';
    renderBookManager(); 
}

function processBulkBookDelete() {
    let checkedBoxes = document.querySelectorAll('.book-delete-checkbox:checked');
    if (checkedBoxes.length === 0) {
        Swal.fire({icon: 'warning', title: 'Kitap Seçilmedi', text: 'Lütfen silmek istediğiniz kitapları seçin.'});
        return;
    }
    
    let namesToDelete = Array.from(checkedBoxes).map(cb => cb.value);
    
    Swal.fire({
        title: 'Emin misiniz?',
        text: `Seçili ${namesToDelete.length} kitabı silmek istediğinize emin misiniz? Bu işlem geri alınamaz!`,
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            namesToDelete.forEach(name => {
                ...
                delete bookPages[name];
            });
            updateUI();
            syncData();
            Swal.fire({icon: 'success', title: 'Silindi', text: 'Seçili kitaplar başarıyla silindi.', timer: 1500, showConfirmButton: false});
        }
    });
}
